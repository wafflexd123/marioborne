...
public class UniInput
{
	...
	readonly List<InputAxis> inputAxes = new List<InputAxis>();

	public UniInput(MonoBehaviour monoBehaviour)
	{
		foreach (string name in Enum.GetNames(typeof(InputAxes))) inputAxes.Add(new InputAxis(this, name, monoBehaviour));
	}
	...
	public class InputAxis
	{
		...
		readonly UniInput uniInput;
		Coroutine crtHold, crtEndPress;

		public InputAxis(UniInput uniInput, string axis, MonoBehaviour monoBehaviour)
		{
		...
		public void Press()
		{
			if (!uniInput.enableInput) return;
		...
		public void Press(Func<float> value, Func<bool> hold)
		{
			...
			IEnumerator Hold()
			{
				...
				do
				{
					...
					direction = value();
					yield return null;
				} while (hold() && uniInput.enableInput);
